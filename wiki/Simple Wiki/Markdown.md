...
►  Якорные внутренние ссылки, например [[Simple Wiki / Markdown # internal-links]], позволяют ссылаться на отдельную часть статьи. Для этого в конец ссылки необходимо добавить соответствующий ID через символ "#" (допускается использовать любое количество уровней и пробелов с каждой стороны).
